...
export const retrieveCustomer = createServerFn({
  method: "GET",
}).handler(async (): Promise<HttpTypes.StoreCustomer | null> => {
...
export const updateCustomer = createServerFn({ method: "POST" })
  .validator((body: HttpTypes.StoreUpdateCustomer) => body)
...
export const signup = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data: formData }): Promise<CustomerAuthState> => {
...
export const confirmEmailVerification = createServerFn({ method: "POST" })
  ...
  .handler(
    async ({ data: token }): Promise<{ success: boolean; error?: string }> => {
      try {
...
export const transferCart = createServerFn({ method: "POST" }).handler(
  async () => {
  ...
  .handler(
    async ({
      ...
      const isDefaultBilling =
        (formData.get("isDefaultBilling") as string) === "true"
      const isDefaultShipping =
        (formData.get("isDefaultShipping") as string) === "true"
...
export const deleteCustomerAddress = createServerFn({ method: "POST" })
  .validator((addressId: string) => addressId)
  .handler(async ({ data: addressId }): Promise<{ success: boolean }> => {
    const headers = {
      ...(await getAuthHeaders()),
    }
