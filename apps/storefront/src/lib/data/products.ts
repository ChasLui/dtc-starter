...
export const listProducts = createServerFn({ method: "GET" })
  ...
  .handler(
    async ({
      data: { pageParam = 1, queryParams, countryCode, regionId },
    }): Promise<{
      ...
      if (countryCode) {
        region = await getRegion({ data: countryCode })
      } else {
        region = await retrieveRegion({ data: regionId! })
      }
 ...
 */
export const listProductsWithSort = createServerFn({ method: "GET" })
  .validator(
    (d: {
      page?: number
      queryParams?: ProductListQueryParams
